...
params:
    ...
    path: "C:/Users/ricardar/Documents/Results/BRICK/calibration-tests/matchingData/calibration-mtch-05-06-fixedB_2025-06-05_10.35.57/"
    scenNames: NULL
    savePlots: false
    name: ""
    customPlots: NULL
...
.createCalibrationPlot(data, "renDevHsPrev", outPath, outName = outName,
                       color = "hs", facets = facets, savePlots = savePlots)
...
if ("renDevSepGabo" %in% data$variable) {
...
.createCalibrationPlot(data, "renDevHsPrevRel", outPath, outName = outName,
                       color = "hs", facets = facets, savePlots = savePlots)
...
if ("renDevSepGaboRel" %in% data$variable) {
...
## Relative deviations by heating system, relative to total target

```{r relative to all stock and flow deviation by hs}

.createCalibrationPlot(data, "stockDevHsRelAll", outPath, outName = outName,
                       color = "hsr", facets = facets, savePlots = savePlots)

.createCalibrationPlot(data, "conDevHsRelAll", outPath, outName = outName,
                       color = "hsr", facets = facets, savePlots = savePlots)

.createCalibrationPlot(data, "renDevHsRelAll", outPath, outName = outName,
                       color = "hsr", facets = facets, savePlots = savePlots)

.createCalibrationPlot(data, "flowDevHsRelAll", outPath, outName = outName,
                       color = "hsr", facets = facets, savePlots = savePlots)

if ("renDevSepGaboRelAll" %in% data$variable) {
  .createCalibrationPlot(data, "renDevSepGaboRelAll", outPath, outName = outName,
                         color = "hsr", facets = facets, savePlots = savePlots,
                         addColor = c(gabo_id = "Gas identical replacement"),
                         newColor = data.frame(row.names = "Gas identical replacement", color = "#FAEBD7"))
}
